      ...
      <div className="z-10 max-w-3xl p-10 md:px-0">
        <Container>
          <h1 className="text-2xl  md:text-4xl  font-bold mb-4 bg-gradient-to-r from-[#BB8819] to-[#F1DD6E] bg-clip-text text-transparent">
            {data.hero.title[locale as keyof LocalizedField]}
          </h1>
          <p className="text-xl mb-6 leading-10 ">
            {data.hero.description[locale as keyof LocalizedField]}
          </p>
          {data.hero.ctaText && (
            <a
            ...
            >
              {data.hero.ctaText[locale as keyof LocalizedField]}
            </a>
          )}
        </Container>
      </div>
